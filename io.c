...
#define PATH_SERVER_FD (TASK_LIMIT + 3) 
...
static int fifo_readable(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
...
static int mq_readable(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
	size_t msg_len;
...
static int fifo_writable(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
...
static int mq_writable(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
...
static int fifo_read(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
	size_t i;
...
static int mq_read(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
	size_t msg_len;
	size_t i;
	char *buf = (char*)task->stack->r1;
...
static int fifo_write(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
	size_t i;
...
static int mq_write(struct pipe_ringbuffer *pipe, struct task_control_block *task)
{
	size_t i;
	const char *buf = (const char*)task->stack->r1;
...
static int mkfile(const char *pathname, int mode, int dev)
{
	...
	*((int *)(buf + 4 + 4 + plen)) = dev;
	write(PATH_SERVER_FD, buf, 4 + 4 + plen + 4);
...
int open(const char *pathname, int flags)
{
	...
	*((unsigned int *)(buf + 4)) = plen;
	memcpy(buf + 4 + 4, pathname, plen);
	write(PATH_SERVER_FD, buf, 4 + 4 + plen);
	read(replyfd, &fd, 4);
...
void pathserver()
{
	...
	while (1) {
		read(PATH_SERVER_FD, &replyfd, 4);
		read(PATH_SERVER_FD, &plen, 4);
		read(PATH_SERVER_FD, path, plen);

		if (!replyfd) { /* mkfifo */
			int dev;
			read(PATH_SERVER_FD, &dev, 4);
